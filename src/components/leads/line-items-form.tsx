...
export type SeedLineItem = { description: string; quantity: string; unitPrice: string };

const seedRow = (item: SeedLineItem): Row => ({
  id: nextId++,
  description: item.description,
  // Fall back to "1" for items saved before quantity was tracked.
...
// Shared row-management UI (add/remove/edit rows, shake-on-invalid,
// dollars<->cents math) behind QuoteForm and InvoiceForm — both need the
// exact same editable line-item table, just pointed at different server
// actions with different seed data/copy.
export function LineItemsForm({
  ...
  const [rows, setRows] = useState<Row[]>(() =>
    seedLineItems && seedLineItems.length > 0 ? seedLineItems.map(seedRow) : [emptyRow()],
  );
    ...
    >
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <Textarea
          ...
          placeholder="Warranty terms, payment instructions, anything else worth printing…"
          rows={2}
